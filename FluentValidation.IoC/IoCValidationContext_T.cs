...
namespace FluentValidation.IoC
{
    public sealed class IoCValidationContext<T>
    {
        ...
        {
            return new IoCValidationContext<T, TValidator>(dependencyResolver.GetValidatorFactory().GetValidator<T, TValidator>());
        }

        public ValidationResult ValidateUsing<TValidator>(T instance)
            where TValidator : IValidator<T>
        {
            var validator = dependencyResolver.GetValidatorFactory().GetValidator<T, TValidator>();
